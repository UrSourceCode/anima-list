...
class _WatchlistScreenState extends State<WatchlistScreen> {
  ...
  void _fetchWatchlist() async {
    try {
      ...
      if (userQuerySnapshot.docs.isNotEmpty) {
        ...
        print('Watchlist for $myUserEmail:');
        for (var item in watchlist) {
          print('AnimeID: ${item['animeID']}, Rating: ${item['rating']}, Status: ${item['listStatus']}');
        }
  ...
  void _deleteItemFromWatchlist(String animeID) async {
    try {
      ...
      if (indexToDelete != -1) {
        watchlist.removeAt(indexToDelete); // Remove the item from the watchlist
  ...
  void _fetchAnimeData(List<Map<String, dynamic>> watchlist) async {
  ...
  void _saveRatingToFirebase(String animeID, int newRating, ListStatus newListStatus, VoidCallback onComplete) async {
    try {
      ...
      if (userQuerySnapshot.docs.isNotEmpty) {
        ...
        List<dynamic> watchlist = userDocument['watchlist'];
                            ...
                            return RatingDialog(
                              ...
                              onSave: (newRating, newListStatus) {
                                _saveRatingToFirebase(item['animeID'], newRating, newListStatus, () {
                                  // Callback to refresh the watchlist
                                  _fetchWatchlist();
                                });
                              },
                            );
